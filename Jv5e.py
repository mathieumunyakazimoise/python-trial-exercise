...
def get_result(hand_diff):
    """Return result string based on hand difference"""
    if hand_diff == 0:
        return 'draw'
...
def view_result(hand_diff):
    """Display result using dictionary"""
...
def play():
    """Main game function with recursion for draws"""
    start_message()
    player = get_player()
    computer = get_computer()
    view_hand(player, computer)
    hand_diff = (player - computer) % 3
    result = view_result(hand_diff)
    
    # If result is draw, play again
    if result == 'draw':
        play()
